...
def ask(
    ...
    try:
        ...
        results = store.search(question, n_results=limit, include_related=True)
        ...
        for result in results:
            ...
            if chunk.symbol_name:
                symbols.append(f"{chunk.symbol_name} ({chunk.symbol_type.value})")

        context = "\n\n".join(context_parts)
        symbols_str = ", ".join(symbols) if symbols else "No specific symbols"
        ...
        for result in results:
            ...
            for rel in rels[:3]:
                relationships.append(f"{rel.source_id} --{rel.relation_type.value}--> {rel.target_id}")
        relationships_str = "\n".join(relationships[:10]) if relationships else "No relationships found"
        ...
        try:
            ...
            if not any(status.values()):
                ...
                answer = router.complete_sync(prompt, system=SYSTEM_PROMPT, max_tokens=1500)
            ...
            for i, result in enumerate(results, 1):
                ...
                if chunk.symbol_name:
                    console.print(f"   Symbol: {chunk.symbol_name}")
